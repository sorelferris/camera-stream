#!/usr/bin/env bash
# Build and optionally publish the standalone camera-stream client distribution.
set -euo pipefail

readonly SCRIPT_DIR="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)"
readonly REPOSITORY_ROOT="$(cd -- "$SCRIPT_DIR/.." && pwd)"
readonly DIST_DIR="$REPOSITORY_ROOT/dist/camera-stream-client"
...
usage() {
  cat <<'EOF'
Usage: scripts/publish_camera_stream_client.sh [OPTIONS]

Build, validate, and publish camera-stream-client.
...
Examples:
  ...
  scripts/publish_camera_stream_client.sh --testpypi --publish
  UV_PUBLISH_TOKEN=pypi-... scripts/publish_camera_stream_client.sh --publish
EOF
}
...
printf 'Validating camera-stream-client for %s...\n' "$TARGET_NAME"
uv run --extra dev pytest -q tests/test_debug_client.py
uv run --extra dev black --check example/camera-stream-client/src tests/test_debug_client.py
uv run --extra dev ruff check example/camera-stream-client/src tests/test_debug_client.py
git diff --check

printf 'Building distributions in %s...\n' "$DIST_DIR"
uv build --package camera-stream-client --out-dir "$DIST_DIR" --clear
...
if $publish; then
  printf 'Uploading camera-stream-client to %s...\n' "$TARGET_NAME"
else
...
if $publish; then
  ...
  printf '  uvx --refresh camera-stream-client --version\n'
else
